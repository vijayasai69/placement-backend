...
import { getGroqStructuredOutputMethod, groqStrictifySchema } from "./utils/groq-schema.js";
	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "jsonSchema") {
			...
			const jsonSchema = isInteropZodSchema(schema) || isSerializableSchema(schema) ? toJsonSchema(schema) : schema;
			...
			outputParser = createContentParser(schema);
			const outputSchema = isInteropZodSchema(schema) || isSerializableSchema(schema) ? toJsonSchema(schema) : schema;
			llm = this.withConfig({
		...
		return assembleStructuredOutputPipeline(llm, outputParser, includeRaw, "ChatGroqStructuredOutput");
	}
};
