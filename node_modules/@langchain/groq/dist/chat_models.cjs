...
const require_groq_schema = require("./utils/groq-schema.cjs");
...
function convertMessagesToGroqParams(messages) {
	return messages.map((message) => {
		...
		if ((0, _langchain_core_messages.isAIMessage)(message) && !!message.tool_calls?.length) completionParam.tool_calls = message.tool_calls.map(_langchain_core_output_parsers_openai_tools.convertLangChainToolCallToOpenAI);
		else {
...
* <br />
*/
var ChatGroq = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	lc_namespace = [
		"langchain",
		"chat_models",
		"groq"
	...
	constructor(modelOrFields, fields) {
		const params = typeof modelOrFields === "string" ? {
			...fields ?? {},
			model: modelOrFields
		} : modelOrFields ?? {};
		super(params);
		...
		this.client = new groq_sdk.default({
			apiKey,
		...
		this.stop = params.stopSequences ?? (typeof params.stop === "string" ? [params.stop] : params.stop) ?? [];
	...
	invocationParams(options, extra) {
		...
		const toReturn = {
			...
			tools: options?.tools?.length ? options.tools.map((tool) => (0, _langchain_core_utils_function_calling.convertToOpenAITool)(tool)) : void 0,
	...
	async *_streamResponseChunks(messages, options, runManager) {
		...
		for await (const data of response) {
			...
			const chunk = _convertDeltaToMessageChunk(choice.delta, role, data, void 0);
	...
	async _generate(messages, options, runManager) {
		...
		if (this.streaming) {
			...
			return {
				generations: Object.entries(finalChunks).sort(([aKey], [bKey]) => parseInt(aKey, 10) - parseInt(bKey, 10)).map(([_, value]) => value),
	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		...
		const method = require_groq_schema.getGroqStructuredOutputMethod(this.model, config?.method);
		let functionName = name ?? "extract";
		let outputParser;
		let llm;
		if (method === "jsonSchema") {
			const schemaName = name ?? "structured_output";
			const description = (0, _langchain_core_utils_types.getSchemaDescription)(schema);
			const jsonSchema = (0, _langchain_core_utils_types.isInteropZodSchema)(schema) || (0, _langchain_core_utils_standard_schema.isSerializableSchema)(schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(schema) : schema;
			const strictSchema = require_groq_schema.groqStrictifySchema(jsonSchema);
			llm = this.withConfig({
				response_format: {
					type: "json_schema",
					json_schema: {
			...
			outputParser = (0, _langchain_core_language_models_structured_output.createContentParser)(schema);
			const outputSchema = (0, _langchain_core_utils_types.isInteropZodSchema)(schema) || (0, _langchain_core_utils_standard_schema.isSerializableSchema)(schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(schema) : schema;
			llm = this.withConfig({
		...
		return (0, _langchain_core_language_models_structured_output.assembleStructuredOutputPipeline)(llm, outputParser, includeRaw, "ChatGroqStructuredOutput");
	}
};
