...
var Pregel = class extends PartialRunnable {
	...
	*/
	async _prepareStateSnapshot({ config, saved, subgraphCheckpointer, applyPendingWrites = false }) {
		...
		const channels = await require_base.channelsFromCheckpoint(this.channels, saved.checkpoint, {
	...
	*/
	async bulkUpdateState(startConfig, supersteps) {
		...
		const updateSuperStep = async (inputConfig, updates) => {
			...
			const channels = await require_base.channelsFromCheckpoint(this.channels, checkpoint, {
				saver: checkpointer,
	...
	*/
	async *_streamIterator(input, options) {
		...
		config.control ??= new require_runtime.RunControl();
	...
	async _runLoop(params) {
		...
		try {
			...
			if (loop.status === "draining") {
				...
				throw new require_errors.GraphDrained(loop.control.drainReason ?? "shutdown");
			}
