...
*/
var StreamProtocolMessagesHandler = class extends _langchain_core_callbacks_base.BaseCallbackHandler {
	...
	normalizeMessageId(message, runId) {
		...
		if (runId != null) if (_langchain_core_messages.ToolMessage.isInstance(message)) messageId ??= `run-${runId}-tool-${message.tool_call_id}`;
		else {
	...
	emitFinalMessage(meta, message, runId, dedupe = false) {
		...
		const toolCallId = role === "tool" && _langchain_core_messages.ToolMessage.isInstance(message) ? message.tool_call_id : void 0;
		this.emit(meta, {
			event: "message-start",
	...
	handleLLMEnd(output, runId) {
		...
		const message = _langchain_core_messages.BaseMessage.isInstance(chatGeneration?.message) ? chatGeneration.message : void 0;
	...
	handleChainStart(_chain, inputs, runId, _parentRunId, tags, metadata, _runType, name) {
		if (metadata !== void 0 && name === metadata.langgraph_node && (tags === void 0 || !tags.includes("langsmith:hidden"))) {
			...
			if (typeof inputs === "object") {
				for (const value of Object.values(inputs)) if ((_langchain_core_messages.BaseMessage.isInstance(value) || _langchain_core_messages.BaseMessageChunk.isInstance(value)) && value.id !== void 0) this.seen[value.id] = value;
				else if (Array.isArray(value)) {
					for (const item of value) if ((_langchain_core_messages.BaseMessage.isInstance(item) || _langchain_core_messages.BaseMessageChunk.isInstance(item)) && item.id !== void 0) this.seen[item.id] = item;
				}
			}
		}
	}
	handleChainEnd(outputs, runId) {
		...
		const emitMessage = (value) => {
			if (_langchain_core_messages.BaseMessage.isInstance(value) && !_langchain_core_messages.ToolMessage.isInstance(value)) this.emitFinalMessage(meta, value, runId, true);
		};
		if (_langchain_core_messages.BaseMessage.isInstance(outputs)) emitMessage(outputs);
...
//#endregion
exports.StreamProtocolMessagesHandler = StreamProtocolMessagesHandler;

//# sourceMappingURL=messages-v2.cjs.map