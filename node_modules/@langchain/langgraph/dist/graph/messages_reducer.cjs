...
function messagesDeltaReducer(state, writes) {
	...
	const stateMsgs = state.length > 0 && _langchain_core_messages.BaseMessage.isInstance(state[0]) ? state : state.map(_langchain_core_messages.coerceMessageLikeToMessage);
	const msgs = flat.map(_langchain_core_messages.coerceMessageLikeToMessage);
