...
exports.matchingAgent = exports.MatchingAgent = void 0;
...
const matchingSchema = zod_1.z.object({
    ...
    recommendationReason: zod_1.z.string(),
});
class MatchingAgent {
    /**
     ...
     */
    formatProjects(projects, maxDescLength = 300) {
        if (!projects)
            return "None";
     ...
     */
    formatExperience(experience, maxDescLength = 300) {
        if (!experience)
            return "None";
    ...
    async compareProfileWithJob(candidateProfile, job) {
        logger_js_1.logger.info(`MatchingAgent comparing profile for job: ${job.title} at ${job.company}`);
...
`;
        try {
            const evaluation = await (0, groq_provider_js_1.getStructuredAIResponse)(prompt, matchingSchema, true // USE BACKGROUND MODEL (llama3-70b-8192) TO AVOID RATE LIMIT COLLISIONS
            );
            return evaluation;
        }
        catch (error) {
            logger_js_1.logger.error(`MatchingAgent failed to compare profile and job via Groq. Using fallback logic:`, error);
            ...
            const missing = job.requiredSkills.filter((s) => !candidateSkillsLower.includes(s.toLowerCase()));
...
exports.MatchingAgent = MatchingAgent;
exports.matchingAgent = new MatchingAgent();
exports.default = exports.matchingAgent;
