...
const matching_agent_js_1 = require("../ai-agents/matching-agent.js");
const logger_js_1 = require("../../utils/logger.js");
class RecommendationService {
    async getRecommendations(userId, profileId) {
        if (!profileId) {
            return [];
        }
        const whereClause = { userId, profileId };
    ...
    async markAsViewed(recommendationId) {
        return prisma_js_1.prisma.recommendation.update({
            where: { id: recommendationId },
            data: { isViewed: true },
        });
    }
    async generateRecommendations(userId, targetProfileId) {
        logger_js_1.logger.info(`Generating recommendations for user: ${userId}`);
        // 1. Fetch Candidate Profile
        const profile = targetProfileId
            ? await prisma_js_1.prisma.candidateProfile.findUnique({ where: { id: targetProfileId } })
            : await prisma_js_1.prisma.candidateProfile.findFirst({ where: { userId }, orderBy: { createdAt: 'desc' } });
        if (!profile) {
        ...
        let relevantJobs = [];
        try {
            relevantJobs = await prisma_js_1.prisma.$queryRawUnsafe(`SELECT id, title, company, location, description, "requiredSkills", "applyLink", "applicationDeadline",
         ...
         LIMIT 30`, userId);
            logger_js_1.logger.info(`Successfully retrieved top ${relevantJobs.length} jobs via pgvector similarity.`);
        }
        catch (vectorError) {
            logger_js_1.logger.warn("pgvector similarity search failed or not configured. Falling back to keyword search:", vectorError);
            // Fallback: Fetch active jobs and match via skill arrays overlap
            const allJobs = await prisma_js_1.prisma.job.findMany({
                where: { isActive: true },
                take: 100,
            });
        ...
        const recommendationsResults = await Promise.all(relevantJobs.map(async (job) => {
            try {
                const evalResult = await matching_agent_js_1.matchingAgent.compareProfileWithJob({
                ...
                if (evalResult) {
                    ...
                    await prisma_js_1.prisma.recommendation.deleteMany({
                        where: {
                            userId,
                            jobId: job.id,
                    ...
                    const rec = await prisma_js_1.prisma.recommendation.create({
                        data: {
                            userId,
                            profileId: profile.id,
                            jobId: job.id,
                ...
                logger_js_1.logger.error(`Failed to generate AI recommendation for Job ID ${job.id}:`, agentError);
            }
            return null;
        }));
        return recommendationsResults.filter((r) => r !== null);
    }
}
